...
int delayus = 2000;//delayMicroseconds(delayus); for each step, 2 ms for 1 step is about 2 ms for 20 microns or 10 mircons per ms

int unit = 53;//53 steps = 1 mm, 1 step = 18.9 microns
...
void geometronAction(char action){
  ...
  if(action == 'A'){
    geometronSequence("eeffffeeaaaa");//up and down, then move 4 mm right
  }
  if(action == 'B'){
    geometronSequence("bbbbbbbbbbbbbbbb");//move 16 mm to the left 
  }
  if(action == 'C'){
    geometronSequence("AAAABcccc");//16 mm to the right, then 16 mm to the left then 4 mm away
  }
  if(action == 'D'){
    geometronSequence("CCCCdddddddddddddddd");//4 mm away 4 times then 16 mm back
  }
  if(action == 'E'){
